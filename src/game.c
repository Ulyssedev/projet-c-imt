...
static SDL_Texture **load_link_textures(SDL_Renderer *ren) {
  // Le malloc permet d'allouer directement et précisément la taille de 8
  // textures, c'est génial
...
Mix_Music *load_aie_sound(const char *filename) {
...
void game_init(GameState *game, SDL_Renderer *ren) {
  ...
  loadBlockingMap("src/res/nes_zelda_overworld_blocking_map.txt",
                  game->blocking_map);
...
static int check_collision(float x1, float y1, int w1, int h1, float x2,
                           float y2, int w2, int h2) {
...
void game_update(GameState *game) {
  ...
  if (game->player.is_attacking &&
      now - game->player.attack_start_time > ATTACK_DURATION) {
    game->player.is_attacking = 0;
  }
  ...
  if (dx != 0) {
    int check_tile_x =
        start_tile_x +
        (int)((new_x + (dx > 0 ? 16 - hitbox_margin : hitbox_margin - 1)) / 16);
  ...
  if (dy != 0) {
    int check_tile_y =
        start_tile_y +
        (int)((new_y + (dy > 0 ? 16 - hitbox_margin : hitbox_margin - 1)) / 16);
  ...
  if (can_move_x) {
    if (new_x < 0 && game->current_room_x > 0) {
      ...
      room_changed = 1;
    } else if (new_x > room_pixel_w - 16 &&
               game->current_room_x < WORLD_ROOMS_X - 1) {
      game->current_room_x++;
      new_x = 0;
      room_changed = 1;
    }
    if (new_x >= 0 && new_x <= room_pixel_w - 16)
      game->player.x = new_x;
  }
  ...
  if (can_move_y) {
    if (new_y < 0 && game->current_room_y > 0) {
      ...
      room_changed = 1;
    } else if (new_y > room_pixel_h - 16 &&
               game->current_room_y < WORLD_ROOMS_Y - 1) {
      game->current_room_y++;
      new_y = 0;
      room_changed = 1;
    }
    if (new_y >= 0 && new_y <= room_pixel_h - 16)
      game->player.y = new_y;
  }
  ...
  for (int i = 0; i < game->enemy_count; i++) {
    ...
    if (next_x < 16 || next_x > room_pixel_w - 32)
      e->dx = -e->dx;
    if (next_y < 16 || next_y > room_pixel_h - 32)
      e->dy = -e->dy;
    ...
    if (now - game->player.last_hit_time > INVINCIBILITY_TIME) {
      if (check_collision(game->player.x + 2, game->player.y + 2, 12, 12, e->x,
                          e->y, 16, 16)) {
        game->player.health--;
        game->player.last_hit_time = now;

        Mix_PlayMusic(game->aie_sound, 1); // Joue notre musique
...
void game_render(GameState *game, SDL_Renderer *ren) {
  ...
  for (int ty = 0; ty < ROOM_HEIGHT_TILES; ty++) {
    for (int tx = 0; tx < ROOM_WIDTH_TILES; tx++) {
      ...
      SDL_Rect dst = {tx * 16 * SCALE_FACTOR, ty * 16 * SCALE_FACTOR,
  ...
  for (int i = 0; i < game->enemy_count; i++) {
    ...
    SDL_Rect dst = {(int)(e->x * SCALE_FACTOR), (int)(e->y * SCALE_FACTOR),
                    16 * SCALE_FACTOR, 16 * SCALE_FACTOR};
    SDL_RenderCopy(ren, tex, NULL, &dst);
  }

  Uint32 now = SDL_GetTicks();
  int is_flashing = (now - game->player.last_hit_time < INVINCIBILITY_TIME) &&
                    ((now / 100) % 2);
  // Le flashing permet d'indiquer les degats
  if (!is_flashing) {
    if (game->player.is_attacking) {
      ...
      case 0:
        dst = (SDL_Rect){px, py, w * SCALE_FACTOR, h * SCALE_FACTOR};
        break;
      case 2:
        dst = (SDL_Rect){px, py - (h - 16) * SCALE_FACTOR, w * SCALE_FACTOR,
                         h * SCALE_FACTOR};
        break;
      case 4:
        dst = (SDL_Rect){px - (w - 16) * SCALE_FACTOR, py, w * SCALE_FACTOR,
                         h * SCALE_FACTOR};
        break;
      case 6:
        dst = (SDL_Rect){px, py, w * SCALE_FACTOR, h * SCALE_FACTOR};
      ...
      SDL_Rect player_dst = {(int)(game->player.x * SCALE_FACTOR),
...
void game_cleanup(GameState *game) {
  ...
  for (int i = 0; i < 8; i++) {
    if (game->link_textures[i])
      SDL_DestroyTexture(game->link_textures[i]);
  }
  free(game->link_textures);

  for (int i = 0; i < 4; i++) {
    if (game->link_sword_textures[i])
      SDL_DestroyTexture(game->link_sword_textures[i]);
  }
  free(game->link_sword_textures);

  for (int i = 0; i < 8; i++) {
    if (game->enemy_textures[i])
      SDL_DestroyTexture(game->enemy_textures[i]);
  }
  free(game->enemy_textures);

  for (int i = 0; i < 10; i++) {
    if (game->imt_textures[i])
      SDL_DestroyTexture(game->imt_textures[i]);
  }
